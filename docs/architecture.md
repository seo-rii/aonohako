...
`scripts/report_toolchain_versions.sh` once per profile. Each matrix leg creates
the required `docker save` archive and SHA256 sidecar before best-effort scanner
exports, then uploads its summary fragment, Syft SBOM JSON or a Syft failure
diagnostic JSON, non-blocking Grype JSON scan, and image archive as artifacts.
...
summary, and republishes the summaries plus image archives as a single bundle
artifact.
