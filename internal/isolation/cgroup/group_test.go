...
func TestValidateParentAcceptsRequiredControllers(t *testing.T) {
	...
	writeFile(t, filepath.Join(parent, "cgroup.subtree_control"), "")
	writeFile(t, filepath.Join(parent, "cgroup.procs"), "")
...
func TestValidateParentRejectsGroupWritableParent(t *testing.T) {
	parent := t.TempDir()
	...
	if !strings.Contains(err.Error(), "group/world writable") {
		t.Fatalf("error %q should mention group/world writable", err)
	}
}
...
func TestValidateParentRejectsNonEmptyParentCgroup(t *testing.T) {
	parent := t.TempDir()
	writeFile(t, filepath.Join(parent, "cgroup.controllers"), "cpu memory pids\n")
	writeFile(t, filepath.Join(parent, "cgroup.subtree_control"), "")
	writeFile(t, filepath.Join(parent, "cgroup.procs"), "123\n")
	mountInfo := filepath.Join(t.TempDir(), "mountinfo")
	writeFile(t, mountInfo, "36 25 0:31 / "+parent+" rw - cgroup2 cgroup rw\n")

	err := ValidateParentAt(parent, mountInfo, []string{"cpu", "memory", "pids"})
	if err == nil {
		t.Fatalf("ValidateParentAt() error = nil, want non-empty cgroup rejection")
	}
	if !strings.Contains(err.Error(), "must not contain processes") {
		t.Fatalf("error %q should mention parent processes", err)
	}
}
...
func TestGroupRemoveWithRetryWaitsForDelayedCleanup(t *testing.T) {
	parent := t.TempDir()
	group, err := CreateRunGroup(parent, "run-retry", Limits{
		MemoryMaxBytes: 64 << 20,
		PidsMax:        16,
	})
	if err != nil {
	...
	removeFakeCgroupFiles(t, group)
	leftover := filepath.Join(group.Path, "leftover")
	...
	if err := group.RemoveWithRetry(200 * time.Millisecond); err != nil {
