...
func TestProtocolAndArchitectureDocsMatchQueueLoggingAndFDSemantics(t *testing.T) {
	...
	if !strings.Contains(architecture, "writing values such\nas `+cpu +memory +pids` to `cgroup.subtree_control`") || !strings.Contains(architecture, "positive `memory.max`") || !strings.Contains(architecture, "`pids.max` values") || !strings.Contains(architecture, "writing its PID to `cgroup.procs`") {
