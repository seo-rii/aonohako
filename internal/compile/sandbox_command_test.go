...
func TestFactorParserGetsOnlyThreadSignalCompatibilityException(t *testing.T) {
	...
	body := string(raw)
	for _, marker := range []string{
		`allowProcesses := commandName != "aonohako-assemblyscript-compile" && commandName != "deno" && commandName != "factor" && commandName != "luajit" && commandName != "qjsc" && !isSyntaxOnlySchemeReader && !isBun`,
		`allowUnixSockets := commandName != "aonohako-assemblyscript-compile" && commandName != "factor" && commandName != "luajit" && commandName != "qjsc" && !isSyntaxOnlySchemeReader && !isChickenCompiler && !isBun`,
		`AllowProcesses:           allowProcesses`,
		`AllowUnixSockets:         allowUnixSockets`,
		`allowThreadSignals := isDotnetLike || commandName == "factor"`,
		`AllowThreadSignals:       allowThreadSignals`,
...
func TestDenoCheckCannotSpawnProcessesButKeepsStartupUnixSockets(t *testing.T) {
	raw, err := os.ReadFile("sandbox_command.go")
	if err != nil {
		t.Fatal(err)
	}
	body := string(raw)
	for _, marker := range []string{
		`commandName != "aonohako-assemblyscript-compile" && commandName != "deno"`,
		`allowUnixSockets := commandName != "aonohako-assemblyscript-compile" && commandName != "factor" && commandName != "luajit"`,
		`AllowProcesses:           allowProcesses`,
		`AllowUnixSockets:         allowUnixSockets`,
	} {
		if !strings.Contains(body, marker) {
			t.Fatalf("Deno compile sandbox contract must contain %q", marker)
		}
	}
}

func TestBunSyntaxCheckGetsNoProcessOrSocketException(t *testing.T) {
	raw, err := os.ReadFile("sandbox_command.go")
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, marker := range []string{
		...
		if !strings.Contains(body, marker) {
			t.Fatalf("Bun compile sandbox contract must contain %q", marker)
		}
	}
	for _, forbidden := range []string{
		`AllowProcesses:           isBun`,
		`AllowUnixSockets:         isBun`,
		`AllowThreadSignals:       isBun`,
		`AllowMemfdCreate:         isBun`,
		`AllowNumaPolicy:          isBun`,
	} {
		if strings.Contains(body, forbidden) {
			t.Fatalf("Bun syntax check unexpectedly receives sandbox privilege %q", forbidden)
		}
	}
}

func TestQuickJSBytecodeCheckCannotSpawnProcessesOrOpenSockets(t *testing.T) {
	raw, err := os.ReadFile("sandbox_command.go")
	if err != nil {
		t.Fatal(err)
	}
	body := string(raw)
	for _, marker := range []string{
		`commandName != "luajit" && commandName != "qjsc"`,
		`AllowProcesses:           allowProcesses`,
		`AllowUnixSockets:         allowUnixSockets`,
	} {
		if !strings.Contains(body, marker) {
			t.Fatalf("QuickJS compile sandbox contract must contain %q", marker)
		}
	}
}
