...
func TestRepositoryCatalogStrengthensNewLanguageSmokeCoverage(t *testing.T) {
	...
	tests := map[string][]string{
		...
		"quickjs":    {"QUICKJS_VERSION=2026-06-04", "QUICKJS_SHA256=b376e839b322978313d929fd20663b11ba58b75df5a46c126dd19ea2fa70ad2a", "sha256sum -c -", `if (0 && has_suffix(module_name, ".so"))`, "examples/fib.so", "make -C /tmp/quickjs-src -j2 qjs qjsc examples/fib.so", "dlopen|dlsym|dlclose", `qjs_status=0; qjs_help="$(qjs --help 2>&1)" || qjs_status=$?`, `test "${qjs_status}" -eq 0 || test "${qjs_status}" -eq 1`, `grep -Fqx "QuickJS version ${QUICKJS_VERSION}"`, `qjsc_status=0; qjsc_help="$(qjsc --help 2>&1)" || qjsc_status=$?`, `test "${qjsc_status}" -eq 0 || test "${qjsc_status}" -eq 1`, `grep -Fqx "QuickJS Compiler version ${QUICKJS_VERSION}"`, "qjsc -s -c -o .cache/quickjs/check.c", "qjs --memory-limit 134217728 --stack-size 1048576 --std Main.js", "test ! -e aonohako-quickjs-compile-leak", "Broken.js", "Reject.js", "InternalError: out of memory"},
