...
func buildCommandWithRuntimeTuning(primaryPath, lang string, req *model.RunRequest, tuning config.RuntimeTuningConfig) []string {
	...
	switch lang {
	case "binary", "go-binary", "mojo-binary", "chicken-scheme", "c3":
	...
	case "zsh":
		return []string{"/usr/bin/zsh", "-d", "-f", primaryPath}
	case "fish":
		return []string{"/usr/bin/fish", "--no-config", "--private", primaryPath}
	case "powershell":
	...
	case "j":
		return []string{"aonohako-j", primaryPath}
	case "lolcode":
