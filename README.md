...
- Carbon is pinned to an official experimental nightly. The image precompiles
  that toolchain's Core objects, then submission compilation emits an object,
  links a native executable, and runs it through the standard binary sandbox.
  With the pinned nightly, `Run` should return `i32` and explicitly return zero;
  a void `Run` can leave the last output byte count as the process exit status.
